...
class binlog_event_consumer
{
	...
	function encode_remap_table($table_id)
	{
		...
		$output .= $this->encode_varint(
			count($map->columns) + 4
		); #column count + 4 new columns (dml_type, uow_id, fv$server_id, fv$gsn)
		...
		$nullable = [0, 0, 0, 0]; #none of the columns we added are nullable
		foreach ($map->columns as $col) {
	...
	protected function next_event()
	{
		...
		if ($this->raw_data === false || $this->raw_data === "") {
			return false;
		}
		$header = substr($this->raw_data, 0, 28); # 28 = (19 * 4 / 3) + 3
		#echo "HEADER: $header LEN: " . strlen($header) . "\n";
		if (!$this->data .= base64_decode($header, true)) {
			die1('base64 decode failed for: ' . $header . "\n");
		}
		...
		if (!$this->data .= base64_decode($body, true)) {
			die1('base64 decode failed for: ' . $body . "\n");
		}
	...
	protected function refresh_mvlog_cache()
	{
		...
		$sql  =
			"SELECT table_schema, table_name, mvlog_name from `{$this->mvlogDB}`.`$this->mvlogs` where active_flag=1";
	...
	protected function encode_row_events(&$table_id, $mode)
	{
		...
		$columns_used = [1, 1, 1, 1]; # front-load the columns used list with the four new columns
	...
	protected function encode_varint($int)
	{
		...
		if (($int >= -32768 && $int <= 65535)) {
			return (chr(252) . pack("S", $int));
		}
		if (($int >= -8388608 && $int <= 8388607)) {
			return (chr(253) . $this->encode_int($int, 24));
		}
	...
	protected function parse_event_header()
	{
		...
		$this->header     =
			(object)unpack("Vtimestamp/Cevent_type/Vserver_id/Vevent_length/Vnext_position/vflags", $data);
	}
		...
		foreach ($data as $col => $data_type) {
			...
			switch ($data_type) {
				...
				case $m['double']:
					...
					$columns[$col]['metadata'] = (object)['size' => $this->cast($data)];
					break;

				case $m['varchar']:
					$data = $this->read(2);
					$this->table_map[$table_id]->raw_metadata .= $data;
					$columns[$col]['metadata'] = (object)['max_length' => $this->cast($data)];
					break;
				...
				case $m['bit']:
					...
					$columns[$col]['metadata'] = (object)['bits' => (bytes * 8) + bits];
					break;
				...
				case $m['newdecimal']:
					...
					$columns[$col]['metadata'] = (object)['precision' => $precision, 'scale' => $scale];
					break;
				...
				case $m['geometry']:
					...
					$columns[$col]['metadata'] = (object)['length_size' => $length_size];
					break;
				...
				case $m['var_string']:
					...
					switch ($real_type) {
						...
						case 'set':
							...
							$size = $this->cast($data);
							$this->table_map[$table_id]->raw_metadata .= $data;
							$columns[$col]['metadata'] = (object)['size' => $size];
						...
						default:
							...
							$this->table_map[$table_id]->raw_metadata .= $data;
							$size                      = $this->cast($data);
							$columns[$col]['metadata'] = (object)['max_length' => $size];
					}
					break;
			}
		}
	...
	protected function parse_row_event($mode = 'insert')
	{
		...
		$table_id = $this->read(6);
		if (empty($this->table_map[$table_id])) {
			die1('ROW IMAGE WITHOUT MAPPING TABLE MAP');
		}
		$this->rows[$table_id]['new'] = ['images' => "", 'columns_used' => []];
		$this->rows[$table_id]['old'] = ['images' => "", 'columns_used' => []];
		$flags                        = $this->read(2);
		$column_count                 = $this->cast($this->read_varint(false));
		$columns_used                 = [];
		if (empty($this->rows[$table_id])) {
			$this->rows[$table_id] = [];
		}
	...
	protected function read_varint($keep_packed = true)
	{
		...
		if ($first_byte == 252) {
			return ($keep_packed ? $data : '') . $this->read(2);
		}
		if ($first_byte == 253) {
			return ($keep_packed ? $data : '') . $this->read(3);
		}
		if ($first_byte == 254) {
			return ($keep_packed ? $data : '') . $this->read(8);
		}
		if ($first_byte == 255) {
			die1('invalid varint length found!\n');
		}
	}
	...
	protected function read_nullstring($keep_null = true)
	{
		...
		while (1) {
			...
			$return .= $char;
			if ($this->data === "") {
				die1("did not find end of string!\n");
			}
		}
		if ($keep_null) {
			$return .= $char;
