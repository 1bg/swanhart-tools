...
class FlexCDC
{
	...
	public function __construct($settings = null, $no_connect = false)
	{
		...
		if (!$this->cmdLine) {
			$this->cmdLine = `which mysqlbinlog`;
		}
		if (!$this->cmdLine) {
...
EOREGEX;
	...
	function new_connection($connection_type)
	{
		...
		switch ($connection_type) {
			case 'source':
				...
				$handle = mysql_connect($S['host'] . ':' . $S['port'], $S['user'], $S['password'], true) or die1(
					'Could not connect to MySQL server:' . mysql_error()
				);

				return $handle;
			case 'dest':
				$handle = mysql_connect($D['host'] . ':' . $D['port'], $D['user'], $D['password'], true) or die1(
					'Could not connect to MySQL server:' . mysql_error()
				);
	...
	public function setup($force = false, $only_table = false)
	{
		...
		if (!mysql_select_db($this->mvlogDB, $this->dest)) {
			my_mysql_query('CREATE DATABASE ' . $this->mvlogDB) or die1(
		...
		if ($only_table === false || $only_table == 'mvlogs') {
			if ($this->table_exists($this->mvlogDB, $this->mvlogs, $this->dest)) {
				...
				my_mysql_query('DROP TABLE `' . $this->mvlogDB . '`.`' . $this->mvlogs . '`;') or die1(
			...
			my_mysql_query(
				...
				,
				$this->dest
			) or die1('COULD NOT CREATE TABLE ' . $this->mvlogs . ': ' . mysql_error($this->dest) . "\n");
		}
		...
		if ($only_table === false || $only_table == 'mview_uow') {
			if (FlexCDC::table_exists($this->mvlogDB, $this->mview_uow, $this->dest)) {
				...
				my_mysql_query('DROP TABLE `' . $this->mvlogDB . '`.`' . $this->mview_uow . '`;') or die1(
			...
			my_mysql_query(
				...
				,
				$this->dest
			) or die1('COULD NOT CREATE TABLE ' . $this->mview_uow . ': ' . mysql_error($this->dest) . "\n");
		...
		if ($only_table === false || $only_table == 'binlog_consumer_status') {
			if (FlexCDC::table_exists($this->mvlogDB, $this->binlog_consumer_status, $this->dest)) {
				...
				my_mysql_query('DROP TABLE `' . $this->mvlogDB . '`.`' . $this->binlog_consumer_status . '`;') or die1(
			...
			my_mysql_query(
				...
				'COULD NOT CREATE TABLE ' . $this->binlog_consumer_status . ': ' . mysql_error($this->dest) . "\n"
			);
	...
	public function capture_changes($iterations = 1)
	{
		...
		while ($iterations <= 0 || ($iterations > 0 && $count < $iterations)) {
			...
			while ($row = mysql_fetch_assoc($stmt)) {
				...
				$execCmdLine = sprintf(
					"%s -R --start-position=%d --stop-position=%d %s",
					$this->cmdLine,
	...
	function initialize_dest()
	{
		...
		$stmt = my_mysql_query(
			"select uow_id, gsn_hwm from {$this->mvlogDB}.{$this->mview_uow} order by uow_id desc limit 1",
			$this->dest
		)
	...
	function cleanup_logs()
	{
		$sql = "DELETE bcs.* FROM `" . $this->binlog_consumer_status
			. "` bcs where exec_master_log_pos >= master_log_size and server_id={$this->serverId} AND master_log_file not in (select log_name from log_list)";
		my_mysql_query($sql, $this->dest) or die1($sql . "\n" . mysql_error() . "\n");
	...
	{
		$sql = sprintf(
			"UPDATE `" . $this->mvlogDB . "`.`" . $this->binlog_consumer_status
			. "` set exec_master_log_pos = %d where master_log_file = '%s' and server_id = %d",
			$this->binlogPosition,
			$this->logName,
			$this->serverId
		);
	...
	function commit_transaction()
	{
		$this->set_capture_pos();
		$sql =
			"UPDATE `{$this->mvlogDB}`.`{$this->mview_uow}` SET `commit_time`=str_to_date('%s','%%y%%m%%d %%H:%%i:%%s'), `gsn_hwm` = %d WHERE `uow_id` = %d";
		$sql = sprintf($sql, rtrim($this->timeStamp), $this->gsn_hwm, $this->uow_id);
		my_mysql_query($sql, $this->dest) or die1(
			'COULD NOT UPDATE ' . $this->mvlogDB . "." . $this->mview_uow . ':' . mysql_error($this->dest) . "\n"
		);
	...
	function create_mvlog($v_schema_name, $v_table_name)
	{
		...
		$exec_sql = " INSERT IGNORE INTO `" . $this->mvlogDB . "`.`" . $this->mvlogs
			. "`( table_schema , table_name , mvlog_name ) values('$v_schema_name', '$v_table_name', '" . $v_schema_name
			. "_" . $v_table_name . "')";
