...
{
	public    $raiseWarnings  = false; #directory with replication metadata
	public    $delimiter      = ';';
	protected $mvlogDB        = 'replication';
	...
	public function __construct($settings = null)
	{
		...
		$this->source = mysql_connect($S['host'] . ':' . $S['port'], $S['user'], $S['password'], true)
		or die('Could not connect to MySQL server:' . mysql_error());
		$this->dest = mysql_connect($D['host'] . ':' . $D['port'], $D['user'], $D['password'], true)
		or die('Could not connect to MySQL server:' . mysql_error());
	}
	...
	*/
		...
		switch (strtoupper($command)) {
			...
			default:
				my_mysql_query($sql, $this->dest) or die('Could not execute [SBR] statement:' . $sql . ':'
