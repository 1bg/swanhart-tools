		...
		switch (strtoupper($tokens[0])) {
			...
			case 'DECLARE':
				...
				if (strtoupper($tokens[2]) != 'CURSOR') {
					...
					$select                           = "";
					$curs_name                        = $tokens[1];
					$proc_cursors[$curs_name]['vars'] = [];
					$tokens                           =
						tokenize($stmt, false); #we need to preserve format for the cursor SQL
			...
			case 'OPEN':
				...
				$cursor_sql = str_replace(['"', '\\'], ['\\"', '\\\\'], $proc_cursors[$tokens[1]]['sql']);
