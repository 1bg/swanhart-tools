...
class FlexCDC
{
	...
	public function __construct($settings = null, $no_connect = false)
	{
		...
		if (isset($this->cmdLine) && !file_exists($this->cmdLine)) {
			$this->cmdLine = false;
		}
		if (!$this->cmdLine) {
			$this->cmdLine = trim(`which mysqlbinlog`);
		}
		if (!$this->cmdLine) {
		...
		if (is_array($this->plugin)) {
			foreach ($this->plugin as $plugin) {
				call_user_func([$plugin, 'plugin_init'], $this);
			}
		}

		if (!empty($settings['flexcdc']['skip_before_update'])) {
			$this->skip_before_update = $settings['flexcdc']['skip_before_update'];
		}
		if (!empty($settings['flexcdc']['mark_updates'])) {
			$this->mark_updates = $settings['flexcdc']['mark_updates'];
		}

		if (!empty($settings['flexcdc']['mvlogs'])) {
			$this->mvlogs = $settings['flexcdc']['mvlogs'];
		}
		if (!empty($settings['flexcdc']['binlog_consumer_status'])) {
			$this->binlog_consumer_status = $settings['flexcdc']['binlog_consumer_status'];
		}
		if (!empty($settings['flexcdc']['mview_uow'])) {
			$this->mview_uow = $settings['flexcdc']['mview_uow'];
		}

		if (!empty($settings['flexcdc']['log_retention_interval'])) {
			$this->log_retention_interval = $settings['flexcdc']['log_retention_interval'];
		}
...
EOREGEX;
	...
	public static function shutdown_plugins($instance)
	{
		if ($instance->plugin) {
			foreach ($instance->plugin as $plugin) {
				call_user_func([$plugin, 'plugin_deinit'], $instance);
			}
		}
	}
	...
	{
		if ($new) {
			return $this->new_connection(SOURCE);
		}
	...
	{
		if ($new) {
			return $this->new_connection(DEST);
		}
		...
		switch ($connection_type) {
			case 'source':
				...
				if (!empty($S['port']) && trim($S['port']) != "") {
					$handle = mysqli_connect($S['host'], $S['user'], $S['password'], null, $S['port']) or die1(
						'Could not connect to MySQL server:' . mysqli_connect_error()
					);
				} else {
					$handle = mysqli_connect($S['host'], $S['user'], $S['password'], null) or die1(
						'Could not connect to MySQL server:' . mysqli_connect_error()
					);
				}
			...
			case 'dest':
				if (!empty($S['port']) && trim($S['port']) != "") {
					$handle = mysqli_connect($D['host'], $D['user'], $D['password'], null, $D['port']) or die1(
						'Could not connect to MySQL server:' . mysqli_error($handle)
					);
				} else {
					$handle = mysqli_connect($D['host'], $D['user'], $D['password'], null) or die1(
						'Could not connect to MySQL server:' . mysqli_error($handle)
					);
				}
				mysqli_set_charset($handle, 'utf8');
	...
	public function table_get_column_names($schema, $table, $invalidate_cache = false)
	{
		...
		$sql =
			'select column_name from information_schema.columns where table_schema="%s" and table_name="%s" and ordinal_position > 4 order by ordinal_position';
	...
	public function table_ordinal_datatype($schema, $table, $pos)
	{
		...
		$sql =
			'select data_type from information_schema.columns where table_schema="%s" and table_name="%s" and ordinal_position="%s"';
	...
	public function table_ordinal_is_unsigned($schema, $table, $pos)
	{
		...
		$log_name = 'mvlog_' . md5(md5($schema) . md5($table));
		$sql      =
			'select (column_type like "%%unsigned%%") as is_unsigned from information_schema.columns where table_schema="%s" and table_name="%s" and ordinal_position=%d';
	...
	public function setup($force = false, $only_table = false)
	{
		...
		if (!mysqli_select_db($this->dest, $this->mvlogDB)) {
			my_mysql_query('CREATE DATABASE ' . $this->mvlogDB, $this->dest) or die1(
				'Could not CREATE DATABASE ' . $this->mvlogDB . "\n"
			);
		...
		if ($only_table === false || $only_table == 'mvlogs') {
			if ($this->table_exists($this->mvlogDB, $this->mvlogs)) {
				if (!$force) {
					trigger_error(
						'Table already exists:' . $this->mvlogs . '. Setup aborted! (use --force to ignore this error)',
						E_USER_ERROR
					);

					return false;
				}
				my_mysql_query('DROP TABLE `' . $this->mvlogDB . '`.`' . $this->mvlogs . '`;', $this->dest) or die1(
					'COULD NOT DROP TABLE: ' . $this->mvlogs . "\n" . mysqli_error($this->dest) . "\n"
				);
			}
			my_mysql_query(
				"CREATE TABLE 
				...
				,
				$this->dest
			) or die1('COULD NOT CREATE TABLE ' . $this->mvlogs . ': ' . mysqli_error($this->dest) . "\n");
		}
		...
		if ($only_table === false || $only_table == 'mview_uow') {
			if ($this->table_exists($this->mvlogDB, $this->mview_uow)) {
				...
				my_mysql_query('DROP TABLE `' . $this->mvlogDB . '`.`' . $this->mview_uow . '`;', $this->dest) or die1(
					'COULD NOT DROP TABLE: ' . $this->mview_uow . "\n" . mysqli_error($this->dest) . "\n"
				);
			}
			my_mysql_query(
				"CREATE TABLE 
				...
				,
				$this->dest
			) or die1('COULD NOT CREATE TABLE ' . $this->mview_uow . ': ' . mysqli_error($this->dest) . "\n");

			my_mysql_query("INSERT INTO `" . $this->mview_uow . "` VALUES (1, NULL, 1);", $this->dest) or die1(
				'COULD NOT INSERT INTO:' . $this->mview_uow . "\n"
			);
		}
		if ($only_table === false || $only_table == 'binlog_consumer_status') {
			if ($this->table_exists($this->mvlogDB, $this->binlog_consumer_status)) {
				if (!$force) {
					trigger_error(
						'Table already exists:' . $this->binlog_consumer_status . '  Setup aborted!',
						E_USER_ERROR
					);
				...
				my_mysql_query(
					'DROP TABLE `' . $this->mvlogDB . '`.`' . $this->binlog_consumer_status . '`;',
					$this->dest
				) or die1(
					'COULD NOT DROP TABLE: ' . $this->binlog_consumer_status . "\n" . mysqli_error($this->dest) . "\n"
				);
			}
			my_mysql_query(
				"CREATE TABLE 
				...
				'COULD NOT CREATE TABLE ' . $this->binlog_consumer_status . ': ' . mysqli_error($this->dest) . "\n"
			);
	...
	public function capture_changes($iterations = 1)
	{
		...
		while ($iterations <= 0 || ($iterations > 0 && $count < $iterations)) {
			...
			while ($row = mysqli_fetch_assoc($stmt)) {
				...
				if ($row['exec_master_log_pos'] < 4) {
					$row['exec_master_log_pos'] = 4;
				}
				if (!$this->hasStopNever) {
					$execCmdLine = sprintf(
						"%s --base64-output=decode-rows -v -R --start-position=%d --stop-position=%d %s",
						$this->cmdLine,
					...
					$execCmdLine = sprintf(
						"%s --base64-output=decode-rows -v -R --start-position=%d --stop-never %s",
						$this->cmdLine,
						$row['exec_master_log_pos'],
						$row['master_log_file']
					);
			...
			if (!empty($this->settings['flexcdc']['sleep_increment'])
				&& !empty($this->settings['flexcdc']['sleep_maximum'])
			) {
				if ($processedLogs) {
					$sleep_time = 0;
				} else {
					$sleep_time += $this->settings['flexcdc']['sleep_increment'];
					$sleep_time = $sleep_time > $this->settings['flexcdc']['sleep_maximum']
						? $this->settings['flexcdc']['sleep_maximum'] : $sleep_time;
	...
	function initialize_dest()
	{
		...
		$stmt = my_mysql_query(
			"select gsn_hwm from {$this->mvlogDB}.{$this->mview_uow} order by uow_id desc limit 1",
			$this->dest
		)
	...
	function get_source_logs()
	{
		...
		while ($row = mysqli_fetch_array($stmt)) {
			if (!$has_logs) {
				my_mysql_query(
					"CREATE TEMPORARY table log_list (log_name char(50), primary key(log_name))",
					$this->dest
				) or die1(mysqli_error($this->dest));
				$has_logs = true;
			}
			$sql = sprintf(
				"INSERT INTO `" . $this->binlog_consumer_status
				. "` (server_id, master_log_file, master_log_size, exec_master_log_pos) values (%d, '%s', %d, 0) ON DUPLICATE KEY UPDATE master_log_size = %d ;",
				$this->serverId,
				$row['Log_name'],
				$row['File_size'],
				$row['File_size']
	...
	{
		$sql = "DELETE bcs.* FROM `" . $this->binlog_consumer_status
			. "` bcs where exec_master_log_pos >= master_log_size and server_id={$this->serverId} AND cast(master_log_file as binary) not in (select CAST(log_name as binary) from log_list)";
		my_mysql_query($sql, $this->dest) or die1($sql . "\n" . mysqli_error($this->dest) . "\n");
	...
	function purge_table_change_history()
	{
		...
		$sql = "select max(uow_id) from {$this->mvlogDB}.{$this->mview_uow} where commit_time <= NOW() - INTERVAL "
			. $this->log_retention_interval;
		...
		if (!trim($next_uow_id)) {
			$uow_id = $uow_id - 1;
		} /* don't purge the last row to avoid losing the gsn_hwm */

		$sql =
			"select concat('`','{$this->mvlogDB}', '`.`', mvlog_name,'`') mvlog_fqn from {$this->mvlogDB}.{$this->mvlogs} where active_flag = 1";
		$stmt = my_mysql_query($sql, $conn) or die1($sql . "\n" . mysqli_error($conn) . "\n");
		$done     = false;
		$iterator = 0;
	...
	function set_capture_pos($commit = false)
	{
		...
		$sql = sprintf(
			"INSERT INTO `" . $this->mvlogDB . "`.`" . $this->binlog_consumer_status
			. "` VALUES(%d, '%s',%d, %d) ON DUPLICATE KEY UPDATE exec_master_log_pos = %d",
			$this->serverId,
			$this->logName,
			0,
			$this->binlogPosition,
		...
		if ($commit) {
			my_mysql_query("COMMIT", $this->dest) or die1("COULD NOT EXEC:\n$sql\n" . mysqli_error($this->dest));
		}
	}

	function start_transaction()
	{
		my_mysql_query("START TRANSACTION", $this->dest) or die1(
			"COULD NOT START TRANSACTION;\n" . mysqli_error($this->dest)
		);
		$this->set_capture_pos();
		$sql = sprintf(
			"INSERT INTO `" . $this->mview_uow . "` values(NULL,str_to_date('%s', '%%y%%m%%d %%H:%%i:%%s'),%d);",
			rtrim($this->timeStamp),
			$this->gsn_hwm
		);
		my_mysql_query($sql, $this->dest) or die1(
			"COULD NOT CREATE NEW UNIT OF WORK:\n$sql\n" . mysqli_error($this->dest)
		);
		...
		if ($this->plugin) {
			foreach ($this->plugin as $plugin) {
				call_user_func([$plugin, 'begin_trx'], $this->uow_id, $this->gsn_hwm, $this);
			}
		}
	}
	...
	function commit_transaction()
	{
		...
		$this->inserts = $this->deletes = $this->tables = [];

		$this->set_capture_pos();
		$sql =
			"UPDATE `{$this->mvlogDB}`.`{$this->mview_uow}` SET `commit_time`=str_to_date('%s','%%y%%m%%d %%H:%%i:%%s'), `gsn_hwm` = %d WHERE `uow_id` = @fv_uow_id";
		$sql = sprintf($sql, rtrim($this->timeStamp), $this->gsn_hwm);
		my_mysql_query($sql, $this->dest) or die('COULD NOT UPDATE ' . $this->mvlogDB . "." . $this->mview_uow . ':'
			. mysqli_error($this->dest) . "\n");
		my_mysql_query("COMMIT", $this->dest) or die1("COULD NOT COMMIT TRANSACTION;\n" . mysqli_error($this->dest));

		if ($this->plugin) {
			foreach ($this->plugin as $plugin) {
				call_user_func([$plugin, 'commit_trx'], $this->uow_id, $this->gsn_hwm, $this);
			}
		}
	...
	{
		$this->inserts = $this->deletes = $this->tables = [];
		my_mysql_query("ROLLBACK", $this->dest) or die1(
			"COULD NOT ROLLBACK TRANSACTION;\n" . mysqli_error($this->dest)
		);
		#update the capture position and commit, because we don't want to keep reading a truncated log
		$this->set_capture_pos();
		my_mysql_query("COMMIT", $this->dest) or die1(
			"COULD NOT COMMIT TRANSACTION LOG POSITION UPDATE;\n" . mysqli_error($this->dest)
		);
		if ($this->plugin) {
			foreach ($this->plugin as $plugin) {
				call_user_func([$plugin, 'rollback_trx'], $this->uow_id, $this);
			}
		}
		$this->uow_id = null;
	}
	...
	function delete_row()
	{
		...
		if ($this->DML == "UPDATE" && $this->plugin) {
			foreach ($this->plugin as $plugin) {
				call_user_func(
					[$plugin, 'update_before'],
					$this->cleanup_row($this->db, $this->base_table, $this->row, true),
					$this->db,
					$this->base_table,
					$this->uow_id,
					$this->gsn_hwm,
		...
		$this->tables[$key] = ['schema' => $this->db, 'table' => $this->base_table];
		if ($this->bulk_insert) {
			if (empty($this->deletes[$key])) {
				$this->deletes[$key] = [];
			}
			...
			$valList =
				"({$this->dml_type}, @fv_uow_id, {$this->binlogServerId},{$this->gsn_hwm}," . implode(",", $row) . ")";
		...
		while ($i < 2) {
			...
			foreach ($tables as $table) {
				...
				foreach ($rows as $the_row) {
					...
					if ($bytes > $allowed) {
						my_mysql_query($sql . $valList, $this->dest) or die1(
							"COULD NOT EXEC SQL:\n$sql\n" . mysqli_error($this->dest) . "\n"
						);
						$valList = "";
					}
	...
	function statement($sql)
	{
        ...
        $sql = rtrim($sql, '*/;!\\');
			...
			case 'USE':
				...
				$this->activeDB = trim(
					str_replace($this->delimiter, '', $this->activeDB),
					'`"'
				); // TODO: parse this better: http://dev.mysql.com/doc/refman/5.6/en/identifiers.html
				break;
			...
			case 'RENAME':
				...
				foreach ($clauses as $clause) {
					...
					$sql = "DELETE from `" . $this->mvlogs
						. "` where table_name='$old_base_table' and table_schema='$old_schema'";

					my_mysql_query($sql, $this->dest) or die1($sql . "\n" . mysqli_error($this->dest) . "\n");
					$sql = "REPLACE INTO `" . $this->mvlogs
						. "` (mvlog_name, table_name, table_schema) values ('$new_log_table', '$new_base_table', '$new_schema')";
					my_mysql_query($sql, $this->dest) or die1($sql . "\n" . mysqli_error($this->dest) . "\n");
			...
			case 'ALTER':
				...
				if ($is_alter_table > -1) {
					...
					foreach ($clauses as $clause) {
						...
						if (!preg_match(
							'/^ORDER|^DISABLE|^ENABLE|^ADD CONSTRAINT|^ADD FOREIGN|^ADD FULLTEXT|^ADD SPATIAL|^DROP FOREIGN|^ADD KEY|^ADD INDEX|^DROP KEY|^DROP INDEX|^ADD PRIMARY|^DROP PRIMARY|^ADD PARTITION|^DROP PARTITION|^COALESCE|^REORGANIZE|^ANALYZE|^CHECK|^OPTIMIZE|^REBUILD|^REPAIR|^PARTITION|^REMOVE/i',
							$clause
						)
						) {
					...
					if ($new_clauses) {
						...
						my_mysql_query($new_alter, $this->dest) or die1(
							$new_alter . "\n" . mysqli_error($this->dest) . "\n"
						);
						if ($new_log_table) {
							$sql = "DELETE from `" . $this->mvlogs
			...
			case 'DROP':
				...
				if ($this->raiseWarnings) {
					trigger_error(
						'Detected DROP on a table!  This may break CDC, particularly if the table is recreated with a different structure.',
						E_USER_WARNING
					);
				}
				break;
	...
	function process_binlog($proc, $lastLine = "")
	{
		...
		while (!feof($proc) || $lastLine !== '') {
			...
			if ($prefix == "ERROR") {
				...
				echo "error from mysqlbinlog: $line\n";
			...
			if ($prefix !== '' && $prefix[0] === '#') {
				...
				if (preg_match(
					...
					if ($prefix == "### I" || $prefix == "### U" || $prefix == "### D") {
						if (preg_match(
							'/### (UPDATE|INSERT|DELETE)(?: INTO| FROM| )\s*([^.]+)\.(.*$)/',
							$line,
							$matches
						)) {
							$this->DML        = $matches[1];
		...
		while ($line = fgets($proc)) {
			...
			if ($line == "### WHERE") {
				if (!empty($this->row)) {
					switch ($mode) {
						case -1:
							if ($this->DML == "DELETE" || (!$this->skip_before_update && $this->DML == "UPDATE")) {
								$this->delete_row();
							}
							break;
						case 1:
	...
	function drop_mvlog($schema, $table)
	{
		...
		$sql = sprintf(
			$sql,
			mysqli_real_escape_string($this->dest, $schema),
			mysqli_real_escape_string($this->dest, $table)
		);
		if (!my_mysql_query($sql, $this->dest)) {
			return false;
		}

		my_mysql_query("BEGIN", $this->dest);
		$sql =
			"DELETE FROM " . $this->mvlogDB . ". " . $this->mvlogs . " where table_schema = '%s' and table_name = '%s'";
		$sql = sprintf(
			$sql,
			mysqli_real_escape_string($this->dest, $schema),
			mysqli_real_escape_string($this->dest, $table)
	...
	function create_mvlog($v_schema_name, $v_table_name)
	{
		...
		$cursor_sql =
			"SELECT COLUMN_NAME, IF(COLUMN_TYPE='TIMESTAMP', 'TIMESTAMP', COLUMN_TYPE) COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='$v_table_name' AND TABLE_SCHEMA = '$v_schema_name'";
		...
		$mv_logname      =
			"`" . $this->mvlogDB . '`.`' . 'mvlog_' . md5(md5($v_schema_name) . md5($v_table_name)) . "`";
		$base_mv_logname = 'mvlog_' . md5(md5($v_schema_name) . md5($v_table_name));

		$v_sql       = FlexCDC::concat(
			'CREATE TABLE IF NOT EXISTS ',
			$mv_logname,
			' ( dml_type INT DEFAULT 0, uow_id BIGINT, `fv$server_id` INT UNSIGNED,fv$gsn bigint, ',
			$v_sql,
			'KEY(uow_id, dml_type) ) ENGINE=INNODB'
		);
		$create_stmt = my_mysql_query($v_sql, $this->dest);
		if (!$create_stmt) {
			die1('COULD NOT CREATE MVLOG. ' . $v_sql . "\n");
		}
		$exec_sql = " INSERT IGNORE INTO `" . $this->mvlogDB . "`.`" . $this->mvlogs
			. "`( table_schema , table_name , mvlog_name ) values('$v_schema_name', '$v_table_name', '"
			. $base_mv_logname . "')";
	...
	protected function cleanup_row($db, $table, &$the_row, $get_column_names = false)
	{
		...
		foreach ($the_row as $pos => $col) {
			...
			switch (trim($datatype)) {
				...
				default:
					if (!is_numeric(trim($col, '')) && strtoupper($col) !== 'NULL') {
